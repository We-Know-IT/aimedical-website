    ...
    <div className="flex h-full flex-col rounded-2xl shadow-xl duration-200 md:h-80 md:w-full md:flex-row lg:h-96">
      {post?.listingImage && (
        <Image
          ...
          className="clip-path-b md:clip-path-r h-64 w-full rounded-2xl object-cover md:h-80 md:w-5/12 lg:h-96"
      ...
      <div className="flex flex-col items-center justify-center md:w-7/12">
        <div className="mb-6 mt-2 flex w-full max-w-lg flex-col items-center justify-center space-y-4 px-6 md:items-end">
          ...
          {post ? (
            <LinkButton className="py-3 px-10" href={`/research/${post.slug}`}>
              Read more
            </LinkButton>
          ) : (
