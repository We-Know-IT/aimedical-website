    ...
    <section className="container mt-6 mb-24 bg-background-primary">
      <div className="flex flex-col items-center justify-center gap-y-4 sm:gap-y-8">
        <div className="flex w-full flex-col items-start space-y-4">
          <Typography variant="h3" className="sm:text-start text-darkgray font-robotoFlex font-normal text-[24px]">
            {title}
          </Typography>
        </div>
        <div>
          ...
          {mounted && isMobile && !showAllMembers && employees.length > 2 && (
            <div className="flex justify-center mt-6">
              <button
                ...
                className="bg-white text-primary w-full px-6 py-3 border border-primary rounded-full font-robotoFlex font-normal hover:bg-primary-hover transition-colors">
                View full team
              </button>
            </div>
          )}
